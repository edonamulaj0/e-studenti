...
export default function FacultyAplikantetPage({ params }) {
  const facultyCode = String(params.faculty || "").toLowerCase();
  const faculty = FACULTIES.find((f) => f.code.toLowerCase() === facultyCode);
  ...
  return <FacultyContent faculty={faculty} />;
}

function FacultyContent({ faculty }) {
